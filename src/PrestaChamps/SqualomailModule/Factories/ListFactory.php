...
namespace PrestaChamps\SqualomailModule\Factories;
 ...
 * Class ListFactory
 *
 * @package PrestaChamps\SqualomailModule\Factories
 */
class ListFactory
{
    /**
     ...
     * @throws \Exception
     */
    public static function make($listName, SqualoMail $squaloMail, \Context $context)
    {
        ...
        $result = $squaloMail->post('lists', $data);
        if ($squaloMail->success()) {
            \Configuration::updateValue('SQUALOMAIL_LIST_NAME', $listName);
            \SqualomailModuleConfig::saveValue(\SqualomailModuleConfig::SQUALOMAIL_LIST_NAME, $listName);
            \SqualomailModuleConfig::saveValue(\SqualomailModuleConfig::SQUALOMAIL_LIST_ID, $result['id']);
            return $result;
        }

        throw new \Exception($squaloMail->getLastError() . $squaloMail->getLastResponse()['body']);
    }
}
