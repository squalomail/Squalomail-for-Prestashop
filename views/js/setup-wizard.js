...
$(document).ready(function () {
    ...
    var statePendingSelect = $('#module-squalomailmoduleconfig-statuses-for-pending');
    var stateRefundedSelect = $('#module-squalomailmoduleconfig-statuses-for-refunded');
    var stateCancelledSelect = $('#module-squalomailmoduleconfig-statuses-for-cancelled');
    var stateShippedSelect = $('#module-squalomailmoduleconfig-statuses-for-shipped');
    var statePaidSelect = $('#module-squalomailmoduleconfig-statuses-for-paid');
    wizard.smartWizard({
        useURLhash: false,
    ...
    $("#oauth2-start").click(function (event) {
        ...
        $.ajax({
            ...
            $("#logged-in-as-container").removeClass("hidden").find("b").text(response.email);
    ...
    function syncCategories() {
        $("#step-9 .progress .progress-bar").css("width", "0%");
        var categoriesBatch = categoryIds.chunk(itemsPerRequest);
        if (categoriesBatch.length < 1) {
            $("#categories-sync-in-progress").hide();
            $("#categories-sync-completed").removeClass('hidden');
        }
        for (var i = 0; i < categoriesBatch.length; i++) {
            let current = i;
            $.ajaxq("categoriesSync", {
                url: wizardUrl,
                type: 'POST',
                data: {
                    action: 'syncCategories',
                    items: categoriesBatch[i]
                },
            }).success(function (response) {
                $("#step-9 #result").html(JSON.stringify(response, null, 2));
                var currentSuccess = parseInt(current + 1) * parseInt(itemsPerRequest);
                currentSuccess = (currentSuccess >= categoryIds.length) ? categoryIds.length : currentSuccess;
                toastr.success(currentSuccess + " / " + categoryIds.length + " completed");
                success = true;
            }).fail(function (xhr, status, error) {
                console.log(xhr);
                ...
                $("#step-9 .progress .progress-bar").css("width", (((current + 1) * parseInt(itemsPerRequest) * 100) / categoryIds.length) + "%");
                if (!$.ajaxq.isRunning("categoriesSync")) {
                    $("#categories-sync-in-progress").hide();
                    $("#categories-sync-completed").removeClass('hidden');
                }
            });
